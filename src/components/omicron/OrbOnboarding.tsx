...
Extrae un perfil profesional ESTIMADO. Responde SOLO JSON válido:
...
Reglas: skills 4-6, axes 0-100 (mínimo 20), seniorLabel real, conservador si no es claro.`;
...
export function OrbOnboarding({ onComplete, onProfileGenerated, onSkillsPreview }: OrbOnboardingProps) {
  ...
  if (typeof localStorage !== 'undefined' && localStorage.getItem(ONBOARDING_KEY)) return null;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setGenerating(true);
    setPhase('processing');
    const quickSkills = quickSkillsFromText(input);
    ...
    try {
      ...
      const resp = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'google/gemma-4-31b-it:free', messages: [{ role: 'system', content: PROFILE_PROMPT }, { role: 'user', content: input.trim() }], max_tokens: 512, temperature: 0.5, response_format: { type: 'json_object' } }),
      });
      const data = await resp.json();
      const text = data?.choices?.[0]?.message?.content ?? '';
      let parsed: GeneratedProfile | null = null;
      try { parsed = JSON.parse(text); } catch { const a = text.indexOf('{'); const b = text.lastIndexOf('}'); if (a >= 0 && b > a) parsed = JSON.parse(text.slice(a, b + 1)); }
      ...
      const fallback: GeneratedProfile = { profession: input.trim().slice(0, 50), years: 3, skills: quickSkills.length > 0 ? quickSkills : ['Profesional', 'Adaptabilidad', 'Trabajo en equipo'], axes: { exec: 40, qual: 35, trans: 25, fund: 30 }, seniorLabel: 'Profesional', summary: `Profesional en ${input.trim().slice(0, 30)}.` };
  ...
  }, [input, generating, onComplete, onProfileGenerated, onSkillsPreview]);
  ...
  const handleMic = useCallback(() => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    recog.onresult = (e: any) => { const r = e.results[e.results.length - 1]; setInput(r[0].transcript); if (r.isFinal) handleSubmit(); };
    recog.start();
  }, [handleSubmit]);
