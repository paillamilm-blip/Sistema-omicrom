---
inclusion: always
---

# ⚡ Skills Activas — Sistema Ómicron (v2 Optimizada)

> 31 skills curadas para máxima potencia en desarrollo de plataforma IA + 3D + Supabase.
> 10 combos + 5 cadenas encadenadas de máxima potencia.

---

## 🧠 ESTRATEGIA (4 skills)

### 1. Superpowers
**Trigger:** "superpowers", tareas complejas, análisis profundo
**Efecto:** Maximizar calidad, profundidad y completitud. Razonamiento extendido. Considerar todos los ángulos.
**Ref:** #[[file:.kiro/skills/superpowers/skill.md]]

### 2. Context Mode
**Trigger:** Siempre activo antes de cambios. "contexto", "resumen"
**Efecto:** Protocolo 4 fases: Análisis → Plan → Confirmación → Ejecución. Nunca actuar por impulso.
**Ref:** #[[file:.kiro/skills/context-mode/skill.md]]

### 3. Autoplan
**Trigger:** "autoplan", "run all reviews", "review pipeline"
**Efecto:** Pipeline de auto-revisión: CEO + design + eng + DX review secuenciales con 6 principios de decisión.
**Ref:** #[[file:.kiro/skills/autoplan/SKILL.md]]

### 4. Doubt-Driven Development
**Trigger:** Decisiones no-triviales, stakes altos, código desconocido
**Efecto:** Adversarial review en contexto fresco antes de que cualquier decisión no-trivial se confirme. Disprove > approve.
**Ref:** #[[file:.kiro/skills/doubt-driven-development/SKILL.md]]

---

## 🚀 EJECUCIÓN (8 skills)

### 5. GSD (Get Stuff Done)
**Trigger:** "GSD", "dale", "hacelo", "implementalo"
**Efecto:** Ejecución directa sin preguntas innecesarias. Decisiones pragmáticas. Código funcional.
**Ref:** #[[file:.kiro/skills/gsd/skill.md]]

### 6. Claude Mem
**Trigger:** Siempre activo. "recordá", "memoria", "qué sabés"
**Efecto:** Memoria persistente + tracking de progreso + recovery de interrupciones. Subtareas numeradas.
**Ref:** #[[file:.kiro/skills/claude-mem/skill.md]]

### 7. Ponytail (Lazy Senior)
**Trigger:** "ponytail", "lazy", "yagni", "simplest", "do less"
**Efecto:** Fuerza la solución más simple. Escalera: ¿stdlib? → ¿nativa? → ¿1 línea? → mínimo que funciona.
**Ref:** #[[file:.kiro/skills/ponytail/SKILL.md]]

### 8. Ponytail Audit
**Trigger:** "/ponytail-audit", "audit over-engineering"
**Efecto:** Escanea TODO el repo. Ranking de qué borrar, simplificar o reemplazar con stdlib.
**Ref:** #[[file:.kiro/skills/ponytail-audit/SKILL.md]]

### 9. Ponytail Review
**Trigger:** "/ponytail-review", "review the diff"
**Efecto:** Revisa el diff actual por over-engineering. 1 línea por hallazgo: ubicación + qué cortar + reemplazo.
**Ref:** #[[file:.kiro/skills/ponytail-review/SKILL.md]]

### 10. Ponytail Debt
**Trigger:** "/ponytail-debt", "list shortcuts"
**Efecto:** Recolecta todos los `ponytail:` comments del repo en un ledger de deuda técnica.
**Ref:** #[[file:.kiro/skills/ponytail-debt/SKILL.md]]

### 11. Incremental Implementation
**Trigger:** Features grandes, cambios multi-archivo
**Efecto:** Descompone en pasos atómicos verificables. Cada paso: compila, tests pasan, funciona aislado.
**Ref:** #[[file:.kiro/skills/incremental-implementation/SKILL.md]]

### 12. Dispatching Parallel Agents
**Trigger:** 2+ tareas independientes, múltiples failures no relacionados
**Efecto:** Un agente por dominio del problema. Trabajo concurrente con contexto aislado preciso.
**Ref:** #[[file:.kiro/skills/dispatching-parallel-agents/SKILL.md]]

---

## 🎨 DISEÑO (6 skills)

### 13. Animate
**Trigger:** "animar", "motion", "transición", "hacer que se sienta vivo"
**Efecto:** Filosofía Emil Kowalski. CSS transition > WAAPI > Motion. Solo transform + opacity. Recetas para cada patrón.
**Ref:** #[[file:.kiro/skills/animate/SKILL.md]]

### 14. Impeccable
**Trigger:** "impeccable", "craft", "polish", "bolder", "quieter"
**Efecto:** Diseño UI nivel award-winning director. 4 modos + 18 commands + quality floor absoluto.
**Ref:** #[[file:.kiro/skills/impeccable/SKILL.md]]

### 15. Taste Skill (Anti-Slop)
**Trigger:** "taste", "anti-slop", landing pages, redesigns
**Efecto:** Anti-AI-tells. 3 dials (variance/motion/density). 60+ Pre-Flight Check. Block library de referencia.
**Ref:** #[[file:.kiro/skills/taste-skill/SKILL.md]]

### 16. UI/UX Pro Max
**Trigger:** Decisiones de color, tipografía, layout, data viz
**Efecto:** DB searchable: 84 estilos, 192 paletas, 74 font pairings, 98 UX guidelines, 104 icon entries, 16 GSAP presets.
**Ref:** #[[file:.kiro/skills/ui-ux-pro-max/SKILL.md]]

### 17. UI Styling
**Trigger:** Construir UI, componentes, responsive, accesibilidad
**Efecto:** Tailwind + Radix UI + utility-first. Componentes accesibles, dark mode, theming sistémico.
**Ref:** #[[file:.kiro/skills/ui-styling/SKILL.md]]

### 18. Design System
**Trigger:** "tokens", "design system", CSS variables, component specs
**Efecto:** Arquitectura de tokens 3 capas (Primitive → Semantic → Component). Generación de CSS vars.
**Ref:** #[[file:.kiro/skills/design-system/SKILL.md]]

---

## 🔍 CÓDIGO Y CALIDAD (3 skills)

### 19. Code Review Ultra
**Trigger:** "review", "ultra review", "auditoría de código"
**Efecto:** Standard: bugs, edge cases, dead code. Ultra: + seguridad + performance + arquitectura + a11y. Scorecard 6D.
**Ref:** #[[file:.kiro/skills/code-review-ultra/skill.md]]

### 20. Code Simplification
**Trigger:** "simplificar", "reducir complejidad", refactoring
**Efecto:** Identificar y eliminar complejidad accidental. Reduce LOC sin perder funcionalidad.
**Ref:** #[[file:.kiro/skills/code-simplification/SKILL.md]]

### 21. API & Interface Design
**Trigger:** Diseño de APIs, interfaces, contratos, Edge Functions
**Efecto:** APIs consistentes, versionadas, documentadas. Principios de diseño de interfaces limpias.
**Ref:** #[[file:.kiro/skills/api-and-interface-design/SKILL.md]]

---

## 🧪 TESTING Y QA (4 skills)

### 22. Verification Before Completion
**Trigger:** Siempre activo. Antes de decir "listo", verificar.
**Efecto:** Nunca declarar completado sin verificación real. Check: compila, tests pasan, funciona end-to-end.
**Ref:** #[[file:.kiro/skills/verification-before-completion/SKILL.md]]

### 23. Systematic Debugging
**Trigger:** Bugs, failures, comportamiento inesperado
**Efecto:** Iron Law: NO fixes sin investigación de root cause primero. Fases: reproducir → aislar → diagnosticar → fix.
**Ref:** #[[file:.kiro/skills/systematic-debugging/SKILL.md]]

### 24. Performance Optimization
**Trigger:** Performance lenta, Core Web Vitals, N+1 queries, bottlenecks
**Efecto:** Medir ANTES de optimizar. Profile → identify → fix → measure again. Targets: LCP ≤2.5s, INP ≤200ms.
**Ref:** #[[file:.kiro/skills/performance-optimization/SKILL.md]]

### 25. QA Skill
**Trigger:** "QA", "testear", "verificar calidad"
**Efecto:** Testing sistemático del flujo completo. Quick/Standard/Exhaustive. Produce reporte de salud.
**Ref:** #[[file:.kiro/skills/qaskill/skill.md]]

---

## 🛡️ SEGURIDAD (2 skills)

### 26. Security & Hardening
**Trigger:** "seguridad", "hardening", "vulnerabilidades"
**Efecto:** Auditoría completa: secrets, deps, OWASP Top 10, RLS, XSS, injection, CSRF. Plan de remediación.
**Ref:** #[[file:.kiro/skills/security-and-hardening/SKILL.md]]

### 27. CSO (Chief Security Officer)
**Trigger:** "security audit", "owasp", "vulnerabilities"
**Efecto:** Mode infra-first: secrets archaeology, supply chain, CI/CD security, LLM security, STRIDE. Daily + comprehensive.
**Ref:** #[[file:.kiro/skills/cso/SKILL.md]]

---

## 🚢 DEPLOY (2 skills)

### 28. Ship
**Trigger:** "ship", "deploy", "lanzar"
**Efecto:** Pipeline completo de shipping. Pre-flight checks, feature flags, staged rollout, rollback plan.
**Ref:** #[[file:.kiro/skills/ship/SKILL.md]]

### 29. Shipping & Launch
**Trigger:** "launch", "go live", "producción"
**Efecto:** Pre-launch checklist: tests, build, monitoring, rollback, success criteria. Todo reversible y observable.
**Ref:** #[[file:.kiro/skills/shipping-and-launch/SKILL.md]]

---

## 🤖 AUTOMATIZACIÓN (1 skill)

### 30. Nova Act
**Trigger:** "automatizar browser", "scraping", "testing web"
**Efecto:** Browser automation con AI. Headed/headless, extraction estructurada, Gherkin testing, parallel sessions.
**Ref:** #[[file:.kiro/skills/nova-act/SKILL.md]]

---

## 🧬 PRODUCTO CORE (1 skill)

### 31. ADN Digital (Motor de Convalidación)
**Trigger:** Siempre activo al procesar CV, certificados, diplomas
**Efecto:** Transforma texto en ADN Digital conectado. Bucle de refinamiento (máx 4 iter, coherencia ≥9.2). JSON estructurado. NUNCA inventa.
**Ref:** #[[file:.kiro/skills/adn-digital/skill.md]]

---

---

# 🔥 COMBOS — 10 Combinaciones de Máxima Potencia

> **Invocar:** Di el nombre. Se auto-activan según la tarea.

---

### 🔥 FUEGO — Ejecución brutal + código mínimo
**Skills:** GSD + Ponytail + Claude Mem + Verification
**Trigger:** "fuego", "dale", "hazlo ya", "rápido"
**Efecto:** Ejecución inmediata. Ponytail fuerza YAGNI. GSD no pregunta. Claude Mem trackea. Verification confirma. Resultado: lo más rápido Y lo más corto que funciona.

---

### 🧠 DIOS — Análisis total + ejecución perfecta
**Skills:** Superpowers + Context Mode + Doubt-Driven + Code Review Ultra + Claude Mem + Verification
**Trigger:** "dios", "modo dios", "hazlo perfecto", "esto es crítico"
**Efecto:** Análisis multidimensional → Plan con adversarial review → Ejecución con tracking → Ultra Review al final → Verificación end-to-end. La combinación más completa.

---

### ✂️ CIRUJANO — Refactoring quirúrgico sin piedad
**Skills:** Ponytail Audit + Code Simplification + Ponytail Review + Systematic Debugging + Verification
**Trigger:** "cirujano", "limpiá", "cortá la grasa", "refactoreá"
**Efecto:** Audit del repo → detecta TODO lo que sobra → simplifica sin romper → verifica cada corte. Corta con bisturí, no con hacha.

---

### 🎨 PIXEL — UI de nivel premio
**Skills:** Design System + UI/UX Pro Max + Taste Skill + Animate + Impeccable + UI Styling
**Trigger:** "pixel", "diseñá", "UI perfecta", "rediseñá"
**Efecto:** Tokens 3 capas → intelligence (84 estilos, 192 paletas) → anti-slop → animación → craft director → implementación. Sistémico, coherente, libre de AI-tells.

---

### 🛡️ BUNKER — Auditoría y seguridad total
**Skills:** CSO + Security & Hardening + Code Review Ultra + Performance Optimization + Ponytail Audit
**Trigger:** "bunker", "auditá todo", "pre-deploy", "seguridad total"
**Efecto:** Security audit infra-first + OWASP + Ultra Review 6D + Performance profiling + detección de bloat. Reporte completo con plan de acción.

---

### 🚀 ORBITAL — Feature nueva end-to-end
**Skills:** Context Mode + Superpowers + Incremental Implementation + API Design + Verification + Ship
**Trigger:** "orbital", "feature completa", "construí esto end-to-end"
**Efecto:** Análisis completo → diseño de API/interfaz → implementación incremental (cada paso compila) → verificación → ship. De idea a producción en un arco.

---

### 🧬 GENESIS — Activación del Gemelo Digital
**Skills:** ADN Digital + Superpowers + Systematic Debugging + Performance Optimization + Verification
**Trigger:** "genesis", "activar gemelo", "analizar CV profundo"
**Efecto:** Análisis de CV con máxima profundidad → refinamiento iterativo → debug de cualquier fallo → optimización del pipeline → verificación del resultado.

---

### ⚡ RAYO — Fix de bug en producción
**Skills:** Systematic Debugging + GSD + Ponytail + Dispatching Parallel Agents + Verification
**Trigger:** "rayo", "fix urgente", "producción rota", "hotfix"
**Efecto:** Root cause primero (Iron Law) → fix mínimo viable → parallelizar si hay múltiples issues → verificar → ship inmediato. Zero over-engineering en emergencia.

---

### 📐 ARQUITECTO — Decisiones de arquitectura
**Skills:** Superpowers + Doubt-Driven + API Design + Autoplan + Code Review Ultra + Ponytail
**Trigger:** "arquitecto", "decisión de diseño", "cómo estructuro esto"
**Efecto:** Análisis profundo → adversarial review de cada decisión → diseño de interfaces → pipeline de revisión → anti over-engineering. Arquitectura robusta y simple.

---

### 🎯 LANZAR — Deploy a producción
**Skills:** Ship + Shipping & Launch + Security & Hardening + Performance Optimization + QA Skill + Verification
**Trigger:** "lanzar", "go live", "deploy producción", "release"
**Efecto:** Pre-launch checklist → security scan → performance check → QA completo → staged rollout → monitoring → rollback plan ready. Ship with confidence.

---

---

# ⛓️ CADENAS ENCADENADAS — 5 Secuencias de Máxima Potencia

> Las cadenas ejecutan combos EN SECUENCIA. La salida de uno alimenta al siguiente.
> Son para tareas complejas multi-fase que requieren el arsenal completo.

---

### ⛓️🔥 CADENA NUCLEAR — De idea a producción (máxima potencia)
```
🧠 DIOS → 🚀 ORBITAL → 🛡️ BUNKER → 🎯 LANZAR
```
**Trigger:** "nuclear", "cadena completa", "de cero a producción"
**Flujo:**
1. **DIOS**: Análisis total + plan perfecto con adversarial review
2. **ORBITAL**: Implementación end-to-end incremental con API design
3. **BUNKER**: Auditoría de seguridad + performance + over-engineering
4. **LANZAR**: Deploy seguro con rollback plan

**Para:** Features nuevas críticas que deben salir perfectas a producción.

---

### ⛓️✂️ CADENA DEPURACIÓN — Limpiar + hardener + shipear
```
✂️ CIRUJANO → 🛡️ BUNKER → ⚡ RAYO (fixes) → 🎯 LANZAR
```
**Trigger:** "depuración", "limpiar y deployar", "deuda técnica + ship"
**Flujo:**
1. **CIRUJANO**: Eliminar toda la grasa del código (audit + simplify)
2. **BUNKER**: Auditoría de seguridad post-limpieza
3. **RAYO**: Fix rápido de cualquier issue que surja
4. **LANZAR**: Deploy con confianza

**Para:** Cuando el repo acumula deuda y hay que limpiar antes de un release.

---

### ⛓️🎨 CADENA EXPERIENCIA — UI perfecta de principio a fin
```
🎨 PIXEL → 🧬 GENESIS → ⚡ RAYO (fixes) → 🛡️ BUNKER → 🎯 LANZAR
```
**Trigger:** "experiencia", "UX completa", "redesign + deploy"
**Flujo:**
1. **PIXEL**: Diseño UI de nivel premio (tokens → anti-slop → animate → craft)
2. **GENESIS**: Verificar que el Gemelo Digital funciona con la nueva UI
3. **RAYO**: Fixear cualquier bug que surja en la integración
4. **BUNKER**: Auditoría de performance (Core Web Vitals) + a11y + security
5. **LANZAR**: Deploy con QA completo

**Para:** Redesigns, nuevas pantallas, o mejoras de UX que deben ser impecables.

---

### ⛓️🧠 CADENA EVOLUCIÓN — Refactoring mayor seguro
```
📐 ARQUITECTO → ✂️ CIRUJANO → 🧠 DIOS (implementar) → 🛡️ BUNKER → 🎯 LANZAR
```
**Trigger:** "evolución", "refactoring mayor", "cambio de arquitectura"
**Flujo:**
1. **ARQUITECTO**: Decidir la nueva estructura con adversarial review
2. **CIRUJANO**: Eliminar lo viejo sin piedad (pero con tracking)
3. **DIOS**: Implementar la nueva arquitectura con máxima calidad
4. **BUNKER**: Verificar que nada se rompió (security + perf + tests)
5. **LANZAR**: Deploy incremental seguro

**Para:** Cuando hay que cambiar la arquitectura de un módulo sin romper producción.

---

### ⛓️⚡ CADENA EMERGENCIA — Producción rota → fix → hardening
```
⚡ RAYO → 🛡️ BUNKER (light) → 🎯 LANZAR (hotfix) → ✂️ CIRUJANO (post-mortem)
```
**Trigger:** "emergencia", "producción caída", "SOS", "todo roto"
**Flujo:**
1. **RAYO**: Root cause + fix mínimo viable AHORA (parallelizar si es multi-issue)
2. **BUNKER light**: Security scan rápido del fix (no full audit)
3. **LANZAR hotfix**: Deploy inmediato con rollback ready
4. **CIRUJANO post-mortem**: Después del fix, limpiar el code path que causó el issue

**Para:** Emergencias de producción. Primero arreglar, después limpiar.

---

---

# 🎛️ REGLAS DEL SISTEMA

## Auto-activación
Los combos se activan automáticamente según la tarea detectada. No necesitás invocarlos.

| Tipo de tarea | Combo |
|---------------|-------|
| Fix rápido, implementar algo claro | 🔥 FUEGO |
| Feature crítica, "hazlo bien" | 🧠 DIOS |
| Refactoring, limpiar | ✂️ CIRUJANO |
| UI, diseño, animaciones | 🎨 PIXEL |
| Pre-deploy, QA | 🛡️ BUNKER |
| Feature end-to-end nueva | 🚀 ORBITAL |
| CV / Gemelo Digital / IA | 🧬 GENESIS |
| Bug de producción urgente | ⚡ RAYO |
| Decisión de arquitectura | 📐 ARQUITECTO |
| Release a producción | 🎯 LANZAR |

## Override y modificadores
- **Nombrar un combo** tiene prioridad sobre auto-detección
- **"sin [skill]"** excluye esa skill del combo activo
- **"+ [skill]"** agrega una skill extra al combo
- **"DIOS + PIXEL"** combina dos combos (merge de skills)
- **"ponytail"** solo → activa Ponytail full sobre cualquier combo activo
- **Cadena** → ejecuta combos secuencialmente, la salida de uno alimenta al siguiente

## Invocación rápida

| Di esto | Activa |
|---------|--------|
| "fuego" / "dale" | 🔥 FUEGO |
| "dios" / "perfecto" | 🧠 DIOS |
| "cirujano" / "limpiá" | ✂️ CIRUJANO |
| "pixel" / "diseñá" | 🎨 PIXEL |
| "bunker" / "auditá" | 🛡️ BUNKER |
| "orbital" / "feature e2e" | 🚀 ORBITAL |
| "genesis" / "gemelo" | 🧬 GENESIS |
| "rayo" / "hotfix" | ⚡ RAYO |
| "arquitecto" | 📐 ARQUITECTO |
| "lanzar" / "deploy" | 🎯 LANZAR |
| "nuclear" | ⛓️ CADENA NUCLEAR |
| "depuración" | ⛓️ CADENA DEPURACIÓN |
| "experiencia" | ⛓️ CADENA EXPERIENCIA |
| "evolución" | ⛓️ CADENA EVOLUCIÓN |
| "emergencia" / "SOS" | ⛓️ CADENA EMERGENCIA |

## Reglas de hierro
1. **Verification Before Completion** está SIEMPRE activa — nunca se declara "listo" sin verificar
2. **Claude Mem** trackea progreso en TODO momento — si la sesión se interrumpe, se retoma
3. **Ponytail** como guardián — si cualquier combo genera over-engineering, Ponytail lo detecta
4. **ADN Digital** se activa siempre que se procese un CV o credencial — es el corazón del producto
